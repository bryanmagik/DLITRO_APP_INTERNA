...
export default function EstadoCajaTab({ turno }: { turno: Turno }) {
  ...
  const cargar = useCallback(async (silent = false) => {
    ...
    try {
      const [pgRes, gaRes, pdRes, peRes] = await Promise.all([
      ...
      const pagoDespachadores = desp.reduce((a, p) => a + p.total_a_pagar, 0);
  ...
  const totalEgresos = data.gastosEfectivo + data.pagoDespachadores;
  const cajaChicaInicial = turno.caja_chica_apertura;
  const efectivoGanancias = data.efectivoVentas - data.gastosEfectivo - data.pagoDespachadores;
